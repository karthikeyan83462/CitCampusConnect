...
const ClubManagement: React.FC = () => {
  ...
  const isManager = user && clubs.some(club => club.club_head_id === user.id);
  const myClubs = isManager
    ? clubs
    : clubs.filter(club =>
        clubMembers.some(m => m.user_id === user?.id && m.club_id === club.id && m.position === 'secretary')
      );
